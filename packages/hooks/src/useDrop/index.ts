...
export interface Options {
  ...
  onUri?: (rul: string, event?: React.DragEvent) => void;
...
export default function useDrop(target: BasicTarget, options: Options = {}) {
  const optionsRef = useLatest(options);

  // 拖动存在的问题：将元素移入子元素会触发dragleave,解决可参考https://stackoverflow.com/a/26459269
  const dragEnterTargetRef = useRef<any>();
  ...
  useEffectWithTarget(
    () => {
      ...
      const onDragEnter = (event: React.DragEvent) => {
        ...
        dragEnterTargetRef.current = event.target;
      ...
      const onDragLeave = (event: React.DragEvent) => {
        if (event.target === dragEnterTargetRef.current) {
