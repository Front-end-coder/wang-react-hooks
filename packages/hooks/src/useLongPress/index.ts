...
export default function useLongPress(
  ...
  useEffectWithTarget(
    () => {
      ...
      function overThreshold(event: EventType) {
      ...
      function onStart(event: EventType) {
        if (hasMoveThreshold) {
          const { clientX, clientY } = getClientPosition(event);
          pervPositionRef.current = {
            x: clientX,
            y: clientY,
          };
        }
      ...
      function onMove(event: EventType) {
      ...
      function onEnd(event: EventType, shouldTriggerClick = false) {
        if (timerRef.current) {
      ...
      function onEndWithClick(event: EventType) {
        onEnd(event, true);
      }
